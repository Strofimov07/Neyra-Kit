...
2. **Watch — bounded window** (default 5 min; state it). Async: no summary yet = *not reviewed yet*, not *clean*. Each push re-triggers a fresh pass. A run reporting **`skipping`** (usage limit, quota exhausted, reviewer disabled) never ran: report "no automated review on this PR" and escalate — the manual `code-reviewer` + `security-reviewer` gate becomes mandatory before merge, not optional.
