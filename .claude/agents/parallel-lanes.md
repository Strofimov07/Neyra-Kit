...
2. **Hard rules in any shared tree** — never `git add -A`/`add .` (stage explicit paths only); never `git checkout`/`switch`/`stash`/`reset --hard`/`clean` in a tree a sibling is using; commit to your own branch only; leave files that aren't yours. Run `git worktree list` before any checkout/reset/rebase, and confirm `git branch --show-current` after a checkout — a checkout blocked by another worktree (a sibling's or the user's) exits non-zero and leaves you on the branch you were already on.
