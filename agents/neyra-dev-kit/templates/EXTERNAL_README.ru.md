...
линтером (`lint-scope.py`) на каждой публикации кита.

> Этот репозиторий — публикуемый артефакт: содержимое синкается из канона
> одной командой (`publish.sh`) и не редактируется здесь руками. Нашёл
> проблему — issue сюда, фикс приедет со следующей версией.
