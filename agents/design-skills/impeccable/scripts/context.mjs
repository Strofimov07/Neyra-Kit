...
async function fetchLatestSkillVersion() {
  // Neyra kit local edit: the vendor version-check phone-home to UPDATE_HOST
  // (impeccable.style) is hard-disabled. Returning null == "no newer version known";
  // every caller already treats null as non-fatal. This kills the only outbound
  // (non-localhost) network call in the skill, for all consumers, regardless of
  // .impeccable/config.json. To restore upstream behavior, re-enable the fetch below.
  return null;
  /* upstream:
  try {
