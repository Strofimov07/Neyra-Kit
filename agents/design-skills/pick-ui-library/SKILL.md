...
when_to_use: >-
  Use whenever a web/frontend task is about to pick or add a library — toasts, dialogs and
  popovers, command menus, OTP inputs, animated numbers, charts, drag and drop, long-list
  virtualization, state, conditional classNames, theming — before installing a dependency or
  hand-rolling one. Check package.json first; recommend one pick from the curated list.
  Web/React surfaces only; not iOS.
---
