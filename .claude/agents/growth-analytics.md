...
5. **Experiment operations** — for Firebase-backed tests, read and snapshot the active template, preserve unrelated conditions, show the exact diff, require explicit confirmation before publish, and record a rollback version. Firebase MCP is a control plane; GA4, BigQuery, or the product event mirror remains the measurement source. Before requesting a Crashlytics report, read `crashlytics_reports_guide` through `firebase_read_resources`. A `full` Firebase profile enables complete administration but still requires per-action confirmation and audit/rollback evidence for mutations.
