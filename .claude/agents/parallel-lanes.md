...
4. **Integrate** — a lane that died (`AGENT FAILED — no report`) is inspected first: `git -C <worktree> status --short` + `git log <base>..<branch>`; unsaved work is salvaged, verified, committed — never redone. Then review each lane (`code-reviewer` + `spec-review`), merge one at a time `--no-ff`, then `verify-runtime` + `regression-scout` on the integrated surface; delete the lane branch + worktree. Conflicts in derived files (generated clients, `.pbxproj`, lockfiles, schema/API hashes) are resolved by regenerating from the merged sources, never by hand-merge or "keep both"; two lanes that each added a migration → the later one re-parents onto the current leaf, no merge migration.
