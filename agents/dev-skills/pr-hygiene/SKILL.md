...
5. **Don't mix concerns** — split unrelated changes into separate commits/PRs.
6. **Clean baseline before starting** — for a new branch/worktree, verify the test baseline is green *before* layering work, so a later failure is attributable to your change, not inherited.
7. **Destructive ops need explicit confirmation** — `git reset --hard`, `git clean -fd`, `branch -D`, `worktree remove`, or discarding a branch require an explicit, typed user confirmation naming what will be lost. Never discard work to "clean up".
