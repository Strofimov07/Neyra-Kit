...
when_to_use: >-
  Use when writing, reviewing, or migrating Swift (`ios/**`) — value-type modeling, Swift 6
  strict concurrency (actors, @concurrent, main-actor-by-default), some/any generics, ARC and
  performance, Swift Testing, macros — or when fixing a concurrency error, hang, data race,
  retain cycle, or performance problem. Language craft only; screen design belongs to
  `swiftui-design-principles`, platform facts to `apple-hig`.
---
