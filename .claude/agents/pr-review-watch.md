...
2. **Watch — bounded window** (default 5 min; state it). Async: no summary yet = *not reviewed yet*, not *clean*. Each push re-triggers a fresh pass. A run reporting **`skipping`** (usage limit, quota exhausted, reviewer disabled) or a **`NEUTRAL`** check conclusion never ran: say the automated control is absent for this PR — the kit gate (`code-reviewer` + the surface's matrix add-ons) is now the only review it gets. "Reviewer did not run" ≠ "no review happened": if the gate already ran on the final diff in this session, cite that run; demand a re-run only when it ran on an earlier revision or not at all.
