...
when_to_use: >-
  Use whenever SwiftUI views, WidgetKit widgets, or any native Apple UI are created or
  modified — spacing, type scale, color, component choice, visual hierarchy. This is the
  iOS counterpart to `design-taste-frontend`: it leads on native surfaces (`ios/**`) and
  never fires on web/CSS. Pair with `apple-hig` for platform facts and
  `design-system-conformance` when a shipped design system already governs the surface.
license: MIT
metadata:
  author: arjitj2
  version: "1.1.1"
