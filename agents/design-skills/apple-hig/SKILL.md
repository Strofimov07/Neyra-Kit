...
name: apple-hig
description: >-
  Fact lookup over a local, greppable copy of Apple's Human Interface Guidelines —
  accessibility and ergonomic minimums (44pt hit targets, Dynamic Type, contrast),
  platform terminology, what a system component can actually do, and the conventions
  App Review checks. A reference corpus, not design direction.
when_to_use: >-
  Use when a native Apple UI question needs a fact rather than an opinion — the
  required minimum for a control, what Apple calls a pattern, which system component
  covers a behavior, or whether a layout meets accessibility minimums. Fires on
  iOS/macOS surfaces (`ios/**`), not web. For how a screen should look, use
  `swiftui-design-principles`; this answers what the platform provides and requires.
