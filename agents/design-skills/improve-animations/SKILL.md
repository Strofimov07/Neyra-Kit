...
when_to_use: >-
  Use when asked to "improve the animations", "audit the motion", or produce a roadmap
  of animation fixes across a web codebase — read-only on source, it writes a prioritized
  audit plus self-contained plans for another agent to execute. Web/CSS surfaces; for a
  single diff use `review-animations`.
---
