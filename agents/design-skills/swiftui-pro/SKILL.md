...
when_to_use: >-
  Use when reading, writing, or reviewing SwiftUI code (`ios/**`) — deprecated and
  soft-deprecated API, navigation and state modelling, view decomposition, performance,
  accessibility. Covers code correctness; `swiftui-design-principles` covers how the
  result looks. Complements `code-reviewer` on the Swift/UI layer rather than replacing it.
license: MIT
metadata:
  author: Paul Hudson
  version: "1.1"
