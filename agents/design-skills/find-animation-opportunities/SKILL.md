...
when_to_use: >-
  Use when the user asks "what could be animated here?" or wants a web surface to
  "feel more alive" — a read-only survey that proposes motion with exact values and
  rejects what should stay static. Web/CSS surfaces; for fixing existing animations use
  `improve-animations` (audit + plans) or `review-animations` (single diff).
---
