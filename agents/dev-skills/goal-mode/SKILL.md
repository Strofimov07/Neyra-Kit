...
| "It's one small task, goal-mode doesn't need the ceremony." | Once goal-mode is invoked, checkpoint 1 runs regardless of task count. If you think it's really one task, say so and hand it back — never run the loop with the checkpoint removed. |
